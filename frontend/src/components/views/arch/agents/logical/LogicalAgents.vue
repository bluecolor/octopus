<template lang="pug">
.col-md-8.col-md-offset-2(v-if="logicalAgents.length > 0")
  .box(style="border-top=0px")
    .box-header.with-border
      h3.box-title {{title}}
      .box-tools.pull-right
        .has-feedback.table-search
          input.form-control.input-sm.search-box(v-model="filter" autofocus=true, type='text', placeholder='Search')
    .box-body.no-padding
      .table-controls
        a.btn.btn-default.btn-sm.js-reload-btn(@click='findLogicalAgents' type='button', data-toggle="tooltip" title="Reload",)
          i.fa.fa-refresh.text-blue.fa-lg
        router-link.js-new-plan.btn.btn-default.btn-sm(to='/logical-agent',data-toggle="tooltip" title="New",)
        ...
        router-link.btn.btn-default.btn-sm(:to="'physical-agent/' + selected[0] + '/clone' ", @click="clone", data-toggle="tooltip" title="Clone", :class="selected.length == 1 ? '':'hidden'")
        ...
        table.table.table-hover
          tbody
            tr(v-for="m in collection")
              ...
              td 
                router-link(:to="'logical-agent/' + m.agentId" ) {{m.name}}
              td {{m.url}}
    .box-footer.clearfix
...
.align-center(v-else)
  div.no-connection(style="width:330px; display: table-cell;vertical-align: middle;text-align: center;")
    ...
    div(style="width:70%; margin-top: 20px;display: inline-block;")
      router-link.btn.btn-block.btn-primary.btn-lg(to='logical-agent') Create Logical Agent
</template>
...
export default {
  ...
  computed: {
    ...
    total () {
      if (_.isEmpty(this.filter)) {
        return this.logicalAgents.length
      }
      const agents = _.filter(this.logicalAgents, agent => {
        return agent.name.toLowerCase().indexOf(this.filter.toLowerCase()) !== -1
      })
      return agents.length
    },
    collection () {
      let agents = this.logicalAgents
      if (!_.isEmpty(this.filter)) {
        agents = _.filter(this.logicalAgents, agent => {
          return agent.name.toLowerCase().indexOf(this.filter.toLowerCase()) !== -1
        })
      }
  ...
  methods: {
    ...
    remove () {
      _.each(this.selected, a => this.deleteLogicalAgent(a))
    },
    deleteAgent (id) {
    },
    exportLogicalAgent () {
