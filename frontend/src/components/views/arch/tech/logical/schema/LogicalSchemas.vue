  ...
  .box(style="border-top=0px")
    ...
    .box-body.no-padding
      .table-controls
        ...
        router-link.btn.btn-default.btn-sm(to='logical-schema',data-toggle="tooltip" title="New",)
        ...
        table.table.table-hover
          tbody
            tr(v-for="m in collection")
              ...
              td 
                router-link(:to="'logical-schema/' + m.internalId" ) {{m.name}}
              td(align="center")
                router-link(:to="'logical-schema/' + m.internalId" data-toggle="tooltip" title="Details")
...
.align-center(v-else)
  div.no-connection(style="width:330px; display: table-cell;vertical-align: middle;text-align: center;")
    ...
    div(style="width:100%; margin-top: 20px;display: inline-block;")
      span.text-gray-harbor(style="font-size:20px;") You do not have any logcal schemas!
    div(style="width:70%; margin-top: 20px;display: inline-block;")
      router-link.btn.btn-block.btn-primary.btn-lg(to='logical-schema') Create Logical Schema
...
export default {
  ...
  computed: {
    ...
    title () {
      return `Logical Schemas - ${this.technology.name}`
    },
    technology () {
      return _.find(this.technologies, {internalId: parseInt(this.technologyId)})
    },
    total () {
      let logicalSchemas = this.logicalSchemas(parseInt(this.technologyId))
      if (_.isEmpty(this.filter)) {
        return logicalSchemas.length
      }
      logicalSchemas = _.filter(logicalSchemas, logicalSchema => {
        return logicalSchema.name.toLowerCase().indexOf(this.filter.toLowerCase()) !== -1
      })
      return logicalSchemas.length
    },
    collection () {
      ...
      return _.slice(logicalSchemas, i, i + this.pageSize)
    }
  },
  watch: {
